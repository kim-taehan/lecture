...
## 애플리케이션 포트의 리턴 타입? Entity vs DTO
- 각 계층은 자기 바로 다음 계층에만 의존해야 한다. 
- 도메인 로직을 가진 엔티티가 컨트롤러로 유출되면 도메인 로직을 담은 메소드를 실행할 수 있다. 
  - open session in view 설정문제
- 헥사고날 아키텍처에서 도메인 계층 밖으로 보낼 떄는 반드시 dto로 반환해야 한다. 

## (반박) 애플리케이션 계층의 리턴 타입은 가능하다면 엔티티로 한다.
- 현대적인 애플리케이션은 완화된 계층형 아키텍처를 주로 사용한다. (relaxed layered architecture)
- 계층형 아키텍처의 목적은 시스템의 관심사를 분리하는 것이다. 이를 통해 결합도 낮추고 재사용성과 유지보수성을 높인다. 
- osiv 가 적용됐다고 하더라도 애플리케이션 리턴시 트랜잭션이 종료된다. 
- DTO 를 리턴하는 방식의 가장 큰 문제는 프레젠테이션 로직이 애플리케이션 레이어로 침투하는 것이다. 
- 엔티티를 돌려주고 어떤 정보를 사용자에게 전달할지 선택하는 뷰로직은 웹 API 어뎁터가 담당한다. 
- 복잡한 리포트성 조회 결과는 DTO 를 리턴 (이 자체가 도메인 로직)